...
test("server-renders the Agent Forest integration UI", async () => {
  ...
  assert.match(html, /고양이 에이전트가 일하는 해변 사무실/);
  assert.match(html, /캠핑 라디오 열기/);
  assert.match(html, /무료 시연 예시/);
  assert.match(html, /버튼 색 바꾸기/);
  assert.match(html, /테스트 확인/);
  assert.match(html, /변경 요약/);
...
test("ships local bridge hooks, responsive styles, and 2.5D assets", async () => {
  ...
  assert.match(css, /\.keycap-menu-button\.is-pressed::before/);
  assert.match(css, /translateY\(7px\)/);
